import { pgTable, unique, bigserial, varchar, text, boolean, timestamp, check, integer, foreignKey, bigint, uuid, inet, index, jsonb, smallint, numeric, uniqueIndex, date, char, time, primaryKey, pgView, pgEnum } from "drizzle-orm/pg-core"
import { sql } from "drizzle-orm"

export const auditActionEnum = pgEnum("audit_action_enum", ['INSERT', 'UPDATE', 'DELETE', 'LOGIN', 'LOGOUT'])
export const backgroundJobStatusEnum = pgEnum("background_job_status_enum", ['PENDING', 'RUNNING', 'COMPLETED', 'FAILED', 'CANCELLED', 'RETRYING'])
export const cartStatusEnum = pgEnum("cart_status_enum", ['ACTIVE', 'ABANDONED', 'CONVERTED', 'EXPIRED'])
export const customerTokenPurposeEnum = pgEnum("customer_token_purpose_enum", ['EMAIL_VERIFICATION', 'PASSWORD_RESET', 'EMAIL_CHANGE', 'PHONE_VERIFICATION'])
export const customerTypeEnum = pgEnum("customer_type_enum", ['registered', 'guest'])
export const deliveryAssignmentStatusEnum = pgEnum("delivery_assignment_status_enum", ['ASSIGNED', 'PICKED_UP', 'IN_TRANSIT', 'DELIVERED', 'FAILED', 'CANCELLED'])
export const deliveryEventTypeEnum = pgEnum("delivery_event_type_enum", ['ASSIGNED', 'PICKED_UP', 'ARRIVED', 'DELIVERED', 'FAILED', 'CANCELLED', 'LOCATION_UPDATE'])
export const inventoryMovementTypeEnum = pgEnum("inventory_movement_type_enum", ['purchase', 'sale', 'reservation', 'release', 'adjustment', 'return', 'loss'])
export const inventoryReferenceEnum = pgEnum("inventory_reference_enum", ['ORDER', 'PURCHASE_ORDER', 'TRANSFER', 'RETURN', 'MANUAL', 'ADJUSTMENT'])
export const invoiceStatusEnum = pgEnum("invoice_status_enum", ['pending', 'issued', 'cancelled', 'failed'])
export const mediaStatusEnum = pgEnum("media_status_enum", ['active', 'inactive', 'archived'])
export const mediaTypeEnum = pgEnum("media_type_enum", ['image', 'video', 'audio', 'document', 'other'])
export const menuLocationEnum = pgEnum("menu_location_enum", ['HEADER', 'FOOTER', 'SIDEBAR', 'MOBILE'])
export const notificationChannelEnum = pgEnum("notification_channel_enum", ['IN_APP', 'EMAIL', 'SMS', 'PUSH'])
export const notificationTypeEnum = pgEnum("notification_type_enum", ['INFO', 'SUCCESS', 'WARNING', 'ERROR', 'ORDER', 'PAYMENT', 'PROMOTION'])
export const orderStatusEnum = pgEnum("order_status_enum", ['created', 'payment_pending', 'paid', 'preparing', 'shipped', 'delivered', 'cancelled', 'refunded'])
export const paymentIntentStatusEnum = pgEnum("payment_intent_status_enum", ['created', 'pending', 'authorized', 'failed', 'cancelled'])
export const paymentMethodEnum = pgEnum("payment_method_enum", ['CARD', 'PSE', 'CASH', 'BANK_TRANSFER', 'NEQUI', 'DAVIPLATA'])
export const paymentStatusEnum = pgEnum("payment_status_enum", ['PENDING', 'COMPLETED', 'FAILED', 'REFUNDED', 'CANCELLED'])
export const promotionConditionTypeEnum = pgEnum("promotion_condition_type_enum", ['MIN_AMOUNT', 'MIN_QUANTITY', 'CUSTOMER_TYPE', 'PRODUCT', 'CATEGORY', 'BRAND', 'PAYMENT_METHOD', 'DELIVERY_ZONE'])
export const promotionOperatorEnum = pgEnum("promotion_operator_enum", ['=', '!=', '>', '<', '>=', '<=', 'IN'])
export const refundStatusEnum = pgEnum("refund_status_enum", ['pending', 'approved', 'rejected', 'cancelled'])
export const reservationReferenceEnum = pgEnum("reservation_reference_enum", ['CART', 'ORDER', 'MANUAL'])
export const reservationStatusEnum = pgEnum("reservation_status_enum", ['active', 'expired', 'released', 'converted'])
export const shipmentStatusEnum = pgEnum("shipment_status_enum", ['PENDING', 'PREPARING', 'SHIPPED', 'IN_TRANSIT', 'DELIVERED', 'FAILED', 'RETURNED', 'CANCELLED'])
...
export const userSessions = pgTable("user_sessions", {
	...
	sessionToken: uuid("session_token").default(sql`uuid_generate_v4()`).notNull(),
...
export const users = pgTable("users", {
	...
	index("idx_users_email").using("btree", table.email.asc().nullsLast().op("citext_ops")).where(sql`(deleted_at IS NULL)`),
...
export const permissions = pgTable("permissions", {
	...
	index("idx_permissions_module").using("btree", table.module.asc().nullsLast().op("text_ops")),
...
export const store = pgTable("store", {
	...
	currencyCode: varchar("currency_code", { length: 3 }).default('COP').notNull(),
...
export const branches = pgTable("branches", {
	...
	deliveryRadiusKm: numeric("delivery_radius_km", { precision: 6, scale:  2 }).default('5.0').notNull(),
	schedule: jsonb(),
	isActive: boolean("is_active").default(true).notNull(),
	createdAt: timestamp("created_at", { withTimezone: true, mode: 'string' }).defaultNow().notNull(),
	updatedAt: timestamp("updated_at", { withTimezone: true, mode: 'string' }).defaultNow().notNull(),
	...
	index("idx_branches_location").using("gist", table.location.asc().nullsLast().op("gist_geography_ops")),
	foreignKey({
	...
	check("branches_delivery_radius_km_check", sql`delivery_radius_km > (0)::numeric`),
]);
...
export const deliveryZones = pgTable("delivery_zones", {
	...
	deliveryType: varchar("delivery_type", { length: 30 }).default('STANDARD').notNull(),
	minimumOrder: numeric("minimum_order", { precision: 12, scale:  2 }).default('0').notNull(),
	estimatedTimeMinutes: integer("estimated_time_minutes").default(60).notNull(),
	isActive: boolean("is_active").default(true).notNull(),
	createdAt: timestamp("created_at", { withTimezone: true, mode: 'string' }).defaultNow().notNull(),
	updatedAt: timestamp("updated_at", { withTimezone: true, mode: 'string' }).defaultNow().notNull(),
	deletedAt: timestamp("deleted_at", { withTimezone: true, mode: 'string' }),
}, (table) => [
	index("idx_delivery_zones_coverage").using("gist", table.coverageArea.asc().nullsLast().op("gist_geography_ops")),
	foreignKey({
			columns: [table.branchId],
			foreignColumns: [branches.id],
			name: "delivery_zones_branch_id_fkey"
		}).onDelete("restrict"),
	check("delivery_zones_minimum_order_check", sql`minimum_order >= (0)::numeric`),
]);
...
export const media = pgTable("media", {
	...
	index("idx_media_media_type").using("btree", table.mediaType.asc().nullsLast().op("enum_ops")),
	index("idx_media_mime_type").using("btree", table.mimeType.asc().nullsLast().op("text_ops")),
	index("idx_media_provider").using("btree", table.provider.asc().nullsLast().op("text_ops")),
	index("idx_media_status").using("btree", table.status.asc().nullsLast().op("enum_ops")),
	index("idx_media_uploaded_by").using("btree", table.uploadedBy.asc().nullsLast().op("int8_ops")),
	foreignKey({
...
export const categories = pgTable("categories", {
	...
	index("idx_categories_parent_id").using("btree", table.parentId.asc().nullsLast().op("int8_ops")).where(sql`(deleted_at IS NULL)`),
	uniqueIndex("idx_categories_slug").using("btree", table.slug.asc().nullsLast().op("text_ops")).where(sql`(deleted_at IS NULL)`),
	foreignKey({
...
export const brands = pgTable("brands", {
	...
	uniqueIndex("idx_brands_slug").using("btree", table.slug.asc().nullsLast().op("text_ops")).where(sql`(deleted_at IS NULL)`),
	foreignKey({
			columns: [table.logoMediaId],
			foreignColumns: [media.id],
...
export const products = pgTable("products", {
	...
	description: text(),
	productType: varchar("product_type", { length: 20 }).default('SIMPLE').notNull(),
	...
	index("idx_products_brand").using("btree", table.brandId.asc().nullsLast().op("int8_ops")).where(sql`(deleted_at IS NULL)`),
	index("idx_products_search").using("gin", sql`to_tsvector('spanish'::regconfig, (((name)::text || ' '::text) `).where(sql`((is_active = true) AND (deleted_at IS NULL))`),
	uniqueIndex("idx_products_slug").using("btree", table.slug.asc().nullsLast().op("text_ops")).where(sql`(deleted_at IS NULL)`),
	foreignKey({
...
export const productImages = pgTable("product_images", {
	...
	uniqueIndex("idx_product_images_unique_cover").using("btree", table.productId.asc().nullsLast().op("int8_ops")).where(sql`(is_cover = true)`),
	foreignKey({
...
export const attributeOptions = pgTable("attribute_options", {
	...
	uniqueIndex("idx_attribute_options_unique").using("btree", table.attributeId.asc().nullsLast().op("int8_ops"), table.value.asc().nullsLast().op("int8_ops")),
	foreignKey({
...
export const productVariants = pgTable("product_variants", {
	...
	currentPrice: numeric("current_price", { precision: 12, scale:  2 }).default('0').notNull(),
	currentComparePrice: numeric("current_compare_price", { precision: 12, scale:  2 }),
	...
	uniqueIndex("idx_product_variants_barcode").using("btree", table.barcode.asc().nullsLast().op("text_ops")).where(sql`(barcode IS NOT NULL)`),
	index("idx_product_variants_current_price").using("btree", table.currentPrice.asc().nullsLast().op("numeric_ops")).where(sql`((is_active = true) AND (deleted_at IS NULL))`),
	uniqueIndex("idx_product_variants_sku").using("btree", table.sku.asc().nullsLast().op("text_ops")),
	foreignKey({
			columns: [table.productId],
			foreignColumns: [products.id],
...
export const variantAttributes = pgTable("variant_attributes", {
	...
	attributeId: bigint("attribute_id", { mode: "number" }).notNull(),
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	attributeOptionId: bigint("attribute_option_id", { mode: "number" }).notNull(),
	createdAt: timestamp("created_at", { withTimezone: true, mode: 'string' }).defaultNow().notNull(),
}, (table) => [
	uniqueIndex("idx_variant_attributes_unique").using("btree", table.variantId.asc().nullsLast().op("int8_ops"), table.attributeId.asc().nullsLast().op("int8_ops")),
	foreignKey({
			columns: [table.attributeId],
			foreignColumns: [attributes.id],
...
export const prices = pgTable("prices", {
	...
	uniqueIndex("idx_prices_active_unique").using("btree", table.productVariantId.asc().nullsLast().op("int8_ops")).where(sql`(end_date IS NULL)`),
	foreignKey({
	...
	check("prices_check", sql`(compare_price IS NULL) OR (compare_price >= price)`),
	check("prices_cost_check", sql`(cost IS NULL) OR (cost >= (0)::numeric)`),
	check("prices_date_check", sql`(end_date IS NULL) OR (end_date > start_date)`),
...
export const taxRates = pgTable("tax_rates", {
	...
	index("idx_tax_rates_active").using("btree", table.isActive.asc().nullsLast().op("bool_ops")),
	index("idx_tax_rates_code").using("btree", table.code.asc().nullsLast().op("text_ops")),
	index("idx_tax_rates_validity").using("btree", table.validFrom.asc().nullsLast().op("date_ops"), table.validTo.asc().nullsLast().op("date_ops")),
	unique("tax_rates_code_key").on(table.code),
	check("tax_rates_check", sql`(valid_to IS NULL) OR (valid_to >= valid_from)`),
	check("tax_rates_rate_check", sql`(rate >= (0)::numeric) AND (rate <= (100)::numeric)`),
]);
...
export const inventory = pgTable("inventory", {
	...
	index("idx_inventory_branch").using("btree", table.branchId.asc().nullsLast().op("int8_ops")),
	index("idx_inventory_branch_stock").using("btree", table.branchId.asc().nullsLast().op("int4_ops"), table.stock.asc().nullsLast().op("int8_ops")).where(sql`(stock > 0)`),
	index("idx_inventory_reorder").using("btree", table.reorderPoint.asc().nullsLast().op("int4_ops")),
	index("idx_inventory_reserved").using("btree", table.branchId.asc().nullsLast().op("int8_ops"), table.productVariantId.asc().nullsLast().op("int8_ops")).where(sql`(reserved_stock > 0)`),
	index("idx_inventory_stock").using("btree", table.stock.asc().nullsLast().op("int4_ops")),
	uniqueIndex("idx_inventory_unique").using("btree", table.productVariantId.asc().nullsLast().op("int8_ops"), table.branchId.asc().nullsLast().op("int8_ops")),
	foreignKey({
			columns: [table.branchId],
			foreignColumns: [branches.id],
...
export const inventoryReservations = pgTable("inventory_reservations", {
	...
	index("idx_inventory_reservation_expiration").using("btree", table.expiresAt.asc().nullsLast().op("timestamptz_ops")),
	index("idx_inventory_reservation_inventory").using("btree", table.inventoryId.asc().nullsLast().op("int8_ops")),
	index("idx_inventory_reservation_reference").using("btree", table.referenceType.asc().nullsLast().op("int8_ops"), table.referenceId.asc().nullsLast().op("enum_ops")),
	index("idx_inventory_reservation_status").using("btree", table.status.asc().nullsLast().op("enum_ops")),
	index("idx_inventory_reservations_cleanup").using("btree", table.status.asc().nullsLast().op("enum_ops"), table.expiresAt.asc().nullsLast().op("timestamptz_ops")).where(sql`(status = 'active'::reservation_status_enum)`),
	foreignKey({
	...
	check("inventory_reservations_check", sql`(released_quantity >= 0) AND (released_quantity <= quantity)`),
...
export const inventoryMovements = pgTable("inventory_movements", {
	...
	index("idx_inventory_movements_inventory").using("btree", table.inventoryId.asc().nullsLast().op("int8_ops")),
	index("idx_inventory_movements_reference").using("btree", table.referenceType.asc().nullsLast().op("int8_ops"), table.referenceId.asc().nullsLast().op("enum_ops")),
...
export const supplierProducts = pgTable("supplier_products", {
	...
	index("idx_supplier_products_preferred").using("btree", table.isPreferred.asc().nullsLast().op("bool_ops")),
	index("idx_supplier_products_supplier").using("btree", table.supplierId.asc().nullsLast().op("int8_ops")),
	index("idx_supplier_products_variant").using("btree", table.productVariantId.asc().nullsLast().op("int8_ops")),
	foreignKey({
			columns: [table.productVariantId],
			foreignColumns: [productVariants.id],
	...
	unique("supplier_products_supplier_id_product_variant_id_key").on(table.supplierId, table.productVariantId),
	check("supplier_products_lead_time_days_check", sql`lead_time_days >= 0`),
	check("supplier_products_purchase_price_check", sql`purchase_price >= (0)::numeric`),
]);
...
export const customers = pgTable("customers", {
	...
	acceptsTermsAt: timestamp("accepts_terms_at", { withTimezone: true, mode: 'string' }),
	...
	index("idx_customers_active").using("btree", table.isActive.asc().nullsLast().op("bool_ops")),
	index("idx_customers_last_activity").using("btree", table.lastActivityAt.asc().nullsLast().op("timestamptz_ops")),
...
export const customerAddresses = pgTable("customer_addresses", {
	...
	index("idx_customer_addresses_default").using("btree", table.customerId.asc().nullsLast().op("int8_ops"), table.isDefault.asc().nullsLast().op("int8_ops")),
	index("idx_customer_addresses_location").using("gist", table.location.asc().nullsLast().op("gist_geography_ops")),
	uniqueIndex("idx_customer_default_address").using("btree", table.customerId.asc().nullsLast().op("int8_ops")).where(sql`(is_default = true)`),
	foreignKey({
...
export const customerSessions = pgTable("customer_sessions", {
	...
	index("idx_customer_sessions_customer").using("btree", table.customerId.asc().nullsLast().op("int8_ops")),
	index("idx_customer_sessions_expiration").using("btree", table.expiresAt.asc().nullsLast().op("timestamptz_ops")),
	foreignKey({
			columns: [table.customerId],
			foreignColumns: [customers.id],
...
export const guestSessions = pgTable("guest_sessions", {
	...
	index("idx_guest_sessions_expiration").using("btree", table.expiresAt.asc().nullsLast().op("timestamptz_ops")),
	index("idx_guest_sessions_last_activity").using("btree", table.lastActivityAt.asc().nullsLast().op("timestamptz_ops")),
	foreignKey({
...
export const customerTokens = pgTable("customer_tokens", {
	...
	expiresAt: timestamp("expires_at", { withTimezone: true, mode: 'string' }).default(sql`(now() + '01:00:00'::interval)`).notNull(),
	usedAt: timestamp("used_at", { withTimezone: true, mode: 'string' }),
	createdAt: timestamp("created_at", { withTimezone: true, mode: 'string' }).defaultNow().notNull(),
}, (table) => [
	index("idx_customer_tokens_customer").using("btree", table.customerId.asc().nullsLast().op("int8_ops")),
	index("idx_customer_tokens_expiration").using("btree", table.expiresAt.asc().nullsLast().op("timestamptz_ops")),
	index("idx_customer_tokens_purpose").using("btree", table.purpose.asc().nullsLast().op("enum_ops")),
	foreignKey({
			columns: [table.customerId],
			foreignColumns: [customers.id],
...
export const carts = pgTable("carts", {
	...
	index("idx_carts_customer").using("btree", table.customerId.asc().nullsLast().op("int8_ops")),
	index("idx_carts_guest").using("btree", table.guestSessionId.asc().nullsLast().op("int8_ops")),
	index("idx_carts_last_activity").using("btree", table.lastActivityAt.asc().nullsLast().op("timestamptz_ops")),
	index("idx_carts_status").using("btree", table.status.asc().nullsLast().op("enum_ops")),
	uniqueIndex("idx_unique_active_customer_cart").using("btree", table.customerId.asc().nullsLast().op("int8_ops")).where(sql`(status = 'ACTIVE'::cart_status_enum)`),
	uniqueIndex("idx_unique_active_guest_cart").using("btree", table.guestSessionId.asc().nullsLast().op("int8_ops")).where(sql`(status = 'ACTIVE'::cart_status_enum)`),
	foreignKey({
			columns: [table.branchId],
			foreignColumns: [branches.id],
	...
	check("cart_owner_check", sql`((customer_id IS NOT NULL) AND (guest_session_id IS NULL)) OR ((customer_id IS NULL) AND (guest_session_id IS NOT NULL))`),
]);
...
export const cartItems = pgTable("cart_items", {
	...
	uniqueIndex("idx_cart_item_unique").using("btree", table.cartId.asc().nullsLast().op("int8_ops"), table.productVariantId.asc().nullsLast().op("int8_ops")),
	index("idx_cart_items_cart").using("btree", table.cartId.asc().nullsLast().op("int8_ops")),
	index("idx_cart_items_variant").using("btree", table.productVariantId.asc().nullsLast().op("int8_ops")),
	foreignKey({
	...
	check("cart_items_discount_amount_check", sql`discount_amount >= (0)::numeric`),
...
export const deliveryTimeSlots = pgTable("delivery_time_slots", {
	...
	index("idx_delivery_slot_branch").using("btree", table.branchId.asc().nullsLast().op("int8_ops")),
	foreignKey({
			columns: [table.branchId],
			foreignColumns: [branches.id],
	...
	check("delivery_time_slots_day_of_week_check", sql`(day_of_week >= 0) AND (day_of_week <= 6)`),
]);
...
export const orders = pgTable("orders", {
	...
	currencyCode: varchar("currency_code", { length: 3 }).default('COP').notNull(),
	exchangeRate: numeric("exchange_rate", { precision: 12, scale:  6 }).default('1.0'),
	...
	shippingTax: numeric("shipping_tax", { precision: 12, scale:  2 }).default('0').notNull(),
	...
	index("idx_orders_branch").using("btree", table.branchId.asc().nullsLast().op("int8_ops")),
	index("idx_orders_customer").using("btree", table.customerId.asc().nullsLast().op("int8_ops")),
	index("idx_orders_customer_status").using("btree", table.customerId.asc().nullsLast().op("int8_ops"), table.status.asc().nullsLast().op("int8_ops")),
	index("idx_orders_reference_code").using("btree", table.referenceCode.asc().nullsLast().op("text_ops")),
	index("idx_orders_status").using("btree", table.status.asc().nullsLast().op("enum_ops")),
	index("idx_orders_status_created").using("btree", table.status.asc().nullsLast().op("enum_ops"), table.createdAt.desc().nullsFirst().op("enum_ops")),
	foreignKey({
			columns: [table.couponId],
			foreignColumns: [coupons.id],
...
export const orderItems = pgTable("order_items", {
	...
	check("order_items_unit_price_net_check", sql`unit_price_net >= (0)::numeric`),
]);
...
export const shipments = pgTable("shipments", {
	...
	shippingTaxAmount: numeric("shipping_tax_amount", { precision: 12, scale:  2 }).default('0'),
	status: shipmentStatusEnum().default('PENDING'),
	createdAt: timestamp("created_at", { withTimezone: true, mode: 'string' }).defaultNow().notNull(),
	updatedAt: timestamp("updated_at", { withTimezone: true, mode: 'string' }).defaultNow().notNull(),
}, (table) => [
	index("idx_shipments_location").using("gist", table.location.asc().nullsLast().op("gist_geography_ops")),
	foreignKey({
	...
	check("shipments_shipping_tax_amount_check", sql`shipping_tax_amount >= (0)::numeric`),
]);
...
export const paymentIntents = pgTable("payment_intents", {
	...
	index("idx_payment_intents_gateway").using("btree", table.providerTransactionId.asc().nullsLast().op("text_ops")).where(sql`(provider_transaction_id IS NOT NULL)`),
	index("idx_payment_intents_order_status").using("btree", table.orderId.asc().nullsLast().op("int8_ops"), table.status.asc().nullsLast().op("int8_ops")).where(sql`(status = ANY (ARRAY['pending'::payment_intent_status_enum, 'created'::payment_intent_status_enum]))`),
...
export const payments = pgTable("payments", {
	...
	index("idx_payments_order").using("btree", table.orderId.asc().nullsLast().op("int8_ops")),
	index("idx_payments_payment_intent").using("btree", table.paymentIntentId.asc().nullsLast().op("int8_ops")),
	foreignKey({
			columns: [table.orderId],
			foreignColumns: [orders.id],
	...
	unique("payments_provider_transaction_id_key").on(table.providerTransactionId),
...
export const refunds = pgTable("refunds", {
	...
	index("idx_refunds_payment").using("btree", table.paymentId.asc().nullsLast().op("int8_ops")),
	foreignKey({
			columns: [table.orderId],
			foreignColumns: [orders.id],
...
export const orderStatusHistory = pgTable("order_status_history", {
	...
	index("idx_order_status_history_order").using("btree", table.orderId.asc().nullsLast().op("int8_ops")),
	foreignKey({
			columns: [table.createdBy],
			foreignColumns: [customers.id],
...
export const deliveryDrivers = pgTable("delivery_drivers", {
	...
	index("idx_delivery_drivers_active").using("btree", table.isActive.asc().nullsLast().op("bool_ops")),
	index("idx_delivery_drivers_available").using("btree", table.isAvailable.asc().nullsLast().op("bool_ops")),
]);

export const deliveryDriverLocations = pgTable("delivery_driver_locations", {
	id: bigserial({ mode: "bigint" }).primaryKey().notNull(),
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	...
	index("idx_delivery_driver_locations_location").using("gist", table.location.asc().nullsLast().op("gist_geography_ops")),
	index("idx_driver_locations_driver").using("btree", table.driverId.asc().nullsLast().op("int8_ops")),
	index("idx_driver_locations_latest").using("btree", table.driverId.asc().nullsLast().op("int8_ops"), table.recordedAt.desc().nullsFirst().op("timestamptz_ops")),
	index("idx_driver_locations_recorded").using("btree", table.recordedAt.asc().nullsLast().op("timestamptz_ops")),
	foreignKey({
			columns: [table.driverId],
			foreignColumns: [deliveryDrivers.id],
			name: "delivery_driver_locations_driver_id_fkey"
		}).onDelete("cascade"),
	check("delivery_driver_locations_heading_degrees_check", sql`(heading_degrees >= 0) AND (heading_degrees <= 359)`),
	check("delivery_driver_locations_speed_kmh_check", sql`speed_kmh >= (0)::numeric`),
]);
...
export const deliveryAssignments = pgTable("delivery_assignments", {
	...
	index("idx_delivery_assignment_driver").using("btree", table.driverId.asc().nullsLast().op("int8_ops")),
	uniqueIndex("idx_delivery_assignment_order").using("btree", table.shipmentId.asc().nullsLast().op("int8_ops")),
	index("idx_delivery_assignment_status").using("btree", table.status.asc().nullsLast().op("enum_ops")),
	uniqueIndex("idx_driver_active_assignment").using("btree", table.driverId.asc().nullsLast().op("int8_ops")).where(sql`(status = ANY (ARRAY['ASSIGNED'::delivery_assignment_status_enum, 'PICKED_UP'::delivery_assignment_status_enum, 'IN_TRANSIT'::delivery_assignment_status_enum]))`),
	foreignKey({
			columns: [table.driverId],
			foreignColumns: [deliveryDrivers.id],
...
export const deliveryEvents = pgTable("delivery_events", {
	...
	index("idx_delivery_events_assignment").using("btree", table.assignmentId.asc().nullsLast().op("int8_ops")),
	index("idx_delivery_events_created").using("btree", table.createdAt.asc().nullsLast().op("timestamptz_ops")),
	index("idx_delivery_events_type").using("btree", table.eventType.asc().nullsLast().op("enum_ops")),
	foreignKey({
...
export const promotions = pgTable("promotions", {
	...
	index("idx_promotions_dates").using("btree", table.startDate.asc().nullsLast().op("timestamptz_ops"), table.endDate.asc().nullsLast().op("timestamptz_ops")),
	index("idx_promotions_priority").using("btree", table.priority.asc().nullsLast().op("int4_ops")),
	foreignKey({
			columns: [table.createdBy],
			foreignColumns: [users.id],
...
export const promotionConditions = pgTable("promotion_conditions", {
	...
	index("idx_promotion_conditions_promotion").using("btree", table.promotionId.asc().nullsLast().op("int8_ops")),
	foreignKey({
...
export const coupons = pgTable("coupons", {
	...
	index("idx_coupon_active").using("btree", table.isActive.asc().nullsLast().op("bool_ops")),
	index("idx_coupon_code").using("btree", table.code.asc().nullsLast().op("text_ops")),
	foreignKey({
			columns: [table.promotionId],
			foreignColumns: [promotions.id],
...
export const couponRedemptions = pgTable("coupon_redemptions", {
	...
	index("idx_coupon_redemption_coupon").using("btree", table.couponId.asc().nullsLast().op("int8_ops")),
	index("idx_coupon_redemption_customer").using("btree", table.customerId.asc().nullsLast().op("int8_ops")),
	index("idx_coupon_redemption_order").using("btree", table.orderId.asc().nullsLast().op("int8_ops")),
	foreignKey({
			columns: [table.couponId],
			foreignColumns: [coupons.id],
	...
	check("coupon_redemptions_discount_amount_check", sql`discount_amount >= (0)::numeric`),
]);
...
export const banners = pgTable("banners", {
	...
	linkTarget: varchar("link_target", { length: 20 }).default('_self').notNull(),
	altText: varchar("alt_text", { length: 255 }),
	description: text(),
	position: integer().default(0),
	isActive: boolean("is_active").default(true).notNull(),
	...
	index("idx_banners_active").using("btree", table.isActive.asc().nullsLast().op("bool_ops")),
	index("idx_banners_dates").using("btree", table.startDate.asc().nullsLast().op("timestamptz_ops"), table.endDate.asc().nullsLast().op("timestamptz_ops")),
	index("idx_banners_position").using("btree", table.position.asc().nullsLast().op("int4_ops")),
	foreignKey({
			columns: [table.mediaId],
			foreignColumns: [media.id],
			name: "banners_media_id_fkey"
		}).onDelete("cascade"),
	check("banner_dates_check", sql`(end_date IS NULL) OR (start_date IS NULL) OR (end_date > start_date)`),
]);
...
export const pages = pgTable("pages", {
	...
	index("idx_pages_active").using("btree", table.isActive.asc().nullsLast().op("bool_ops")),
...
export const menus = pgTable("menus", {
	...
	unique("menus_location_key").on(table.location),
]);
...
export const menuItems = pgTable("menu_items", {
	...
	index("idx_menu_items_menu").using("btree", table.menuId.asc().nullsLast().op("int8_ops")),
	index("idx_menu_items_parent").using("btree", table.parentId.asc().nullsLast().op("int8_ops")),
	index("idx_menu_items_position").using("btree", table.menuId.asc().nullsLast().op("int4_ops"), table.position.asc().nullsLast().op("int8_ops")),
	foreignKey({
...
export const auditLogs = pgTable("audit_logs", {
	...
	index("idx_audit_logs_record").using("btree", table.tableName.asc().nullsLast().op("int8_ops"), table.recordId.asc().nullsLast().op("int8_ops")),
	index("idx_audit_logs_table").using("btree", table.tableName.asc().nullsLast().op("text_ops")),
	index("idx_audit_logs_user").using("btree", table.performedBy.asc().nullsLast().op("int8_ops")),
	foreignKey({
...
export const notifications = pgTable("notifications", {
	...
	index("idx_notifications_customer").using("btree", table.targetCustomerId.asc().nullsLast().op("int8_ops")),
	index("idx_notifications_customer_unread").using("btree", table.targetCustomerId.asc().nullsLast().op("bool_ops"), table.isRead.asc().nullsLast().op("bool_ops")).where(sql`(is_read = false)`),
	index("idx_notifications_read").using("btree", table.isRead.asc().nullsLast().op("bool_ops")),
	index("idx_notifications_user").using("btree", table.targetUserId.asc().nullsLast().op("int8_ops")),
	foreignKey({
			columns: [table.targetCustomerId],
			foreignColumns: [customers.id],
			name: "notifications_target_customer_id_fkey"
		}).onDelete("cascade"),
	check("notification_target_check", sql`((target_user_id IS NOT NULL) AND (target_customer_id IS NULL)) OR ((target_user_id IS NULL) AND (target_customer_id IS NOT NULL))`),
]);

export const backgroundJobs = pgTable("background_jobs", {
	id: bigserial({ mode: "bigint" }).primaryKey().notNull(),
	jobName: varchar("job_name", { length: 100 }).notNull(),
	queueName: varchar("queue_name", { length: 50 }).default('default').notNull(),
	...
	scheduledAt: timestamp("scheduled_at", { withTimezone: true, mode: 'string' }).defaultNow(),
	...
	index("idx_background_jobs_priority").using("btree", table.priority.asc().nullsLast().op("int4_ops")),
	index("idx_background_jobs_queue").using("btree", table.queueName.asc().nullsLast().op("text_ops")),
	index("idx_background_jobs_schedule").using("btree", table.scheduledAt.asc().nullsLast().op("timestamptz_ops")),
	index("idx_background_jobs_status").using("btree", table.status.asc().nullsLast().op("enum_ops")),
	check("background_jobs_check", sql`attempts <= max_attempts`),
	check("background_jobs_priority_check", sql`(priority >= 1) AND (priority <= 10)`),
]);
...
export const rolePermissions = pgTable("role_permissions", {
	...
	index("idx_role_permissions_role").using("btree", table.roleId.asc().nullsLast().op("int8_ops")),
	foreignKey({
...
export const userRoles = pgTable("user_roles", {
	...
	index("idx_user_roles_role").using("btree", table.roleId.asc().nullsLast().op("int8_ops")),
	index("idx_user_roles_user").using("btree", table.userId.asc().nullsLast().op("int8_ops")),
	foreignKey({
			columns: [table.roleId],
			foreignColumns: [roles.id],
...
export const productCategories = pgTable("product_categories", {
	...
	index("idx_product_categories_category").using("btree", table.categoryId.asc().nullsLast().op("int8_ops")),
	foreignKey({
...
export const favorites = pgTable("favorites", {
	...
	primaryKey({ columns: [table.customerId, table.productId], name: "favorites_pkey"}),
]);

export const productTaxClasses = pgTable("product_tax_classes", {
	priority: smallint().default(1),
	...
	primaryKey({ columns: [table.productId, table.taxRateId], name: "product_tax_classes_pkey"}),
]);
...
export const promotionProducts = pgTable("promotion_products", {
	...
	primaryKey({ columns: [table.promotionId, table.productVariantId], name: "promotion_products_pkey"}),
	check("promotion_discount_check", sql`((is_percentage = true) AND (discount_value <= (100)::numeric)) OR (is_percentage = false)`),
	check("promotion_products_discount_value_check", sql`discount_value >= (0)::numeric`),
]);
