...
async function seed() {
  ...
  try {
    // 1. Verificar si ya hay productos (para evitar duplicar seed)
    const result = await db.execute(sql`SELECT COUNT(*) FROM products`);
    const count = parseInt(result.rows[0].count);

    if (count > 0) {
      console.log(`✅ Ya existen ${count} productos. Omitiendo seed.`);
      await pool.end();
      return;
    }
    ...
    await db.execute(sql`
      ...
      VALUES (1, 'ACTIVE', 'Mi Tienda S.A.S.', 'Mi Tienda', '900123456-7', 'RESPONSABLE_IVA',
    ...
    await db.execute(sql`
      INSERT INTO users (id, email, username, password_hash, first_name, last_name, is_superuser, is_active)
      VALUES (1, 'admin@mitienda.com', 'admin', '$2a$10$H7zPZq7Y5Z9Z8Z7Z6Z5Z4O', 'Admin', 'Principal', true, true)
      ON CONFLICT (id) DO NOTHING
    `);

    console.log('📦 Insertando sucursal...');
    await db.execute(sql`
      INSERT INTO branches (code, name, address, city, phone, store_id, email, manager_name, manager_phone,
        location, delivery_radius_km, schedule, is_active)
      VALUES ('SUC-001', 'Sucursal Principal', 'Calle 123 # 45-67, Centro', 'Manizales',
        '6012345678', 1, 'sucursal@mitienda.com', 'Carlos Gómez', '3001234567',
    ...
    console.log('📦 Insertando precios históricos...');
    await db.execute(sql`
      INSERT INTO prices (
    ...
    console.log('📦 Asociando imágenes a productos...');
    await db.execute(sql`
      INSERT INTO product_images (product_id, media_id, is_cover, position)
      SELECT p.id, m.id, 
        CASE WHEN m.checksum LIKE '%cover%' THEN true ELSE false END,
        0
      FROM products p
    ...
    console.log('📦 Insertando relaciones producto-categoría...');
    await db.execute(sql`
    ...
    console.log('📦 Insertando categorías principales...');
    await db.execute(sql`
      INSERT INTO product_categories (product_id, category_id)
      SELECT p.id, c.id
      FROM products p
    ...
    await db.execute(sql`
      INSERT INTO inventory (product_variant_id, branch_id, stock, reserved_stock, reorder_point, target_stock, minimum_stock, maximum_stock)
      SELECT 
        pv.id,
        1,
    ...
    await db.execute(sql`
      INSERT INTO suppliers (code, legal_name, tax_id, contact_name, email, phone, address, city, country, is_active) VALUES
    ...
    console.log('📦 Insertando atributos...');
    await db.execute(sql`
    ...
    console.log('📦 Insertando opciones de atributos...');
    await db.execute(sql`
    ...
    console.log('📦 Insertando relaciones variante-atributo...');
    await db.execute(sql`
      ...
      CROSS JOIN LATERAL (
        VALUES 
          ('SGS24U-BK-256', 'Negro', '256GB'),
          ('SGS24U-WH-256', 'Blanco', '256GB'),
          ('SX1VI-BK-256', 'Negro', '256GB'),
          ('SX1VI-SL-256', 'Plata', '256GB'),
          ('SGB3P-WH', 'Blanco', NULL),
          ('SGB3P-BK', 'Negro', NULL),
          ('SVZ-BK-1TB', 'Negro', '1TB'),
          ('SGB4U-SL-1TB', 'Plata', '1TB'),
          ('SWX5-BK', 'Negro', NULL),
          ('SWX5-SL', 'Plata', NULL)
