...
export default function App() {
  ...
  useEffect(() => { localStorage.setItem("checkout_payment", checkoutPayment); }, [checkoutPayment]);
const [landingLoading, setLandingLoading] = useState(true);
  ...
  const placeOrder = async () => {
    ...
    try {
      ...
      setOrders(prev => [o, ...prev]); setLastOrderId(response.referenceCode); setCartItems([]); setOrderPending(response.status === "payment_pending"); setCheckoutStep(4);
  ...
  return (
    <div className="min-h-screen bg-background" style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      {currentView === "account" && (
        <UserAdminView appOrders={orders} cartItems={cartItems} customer={customer} onAdd={addToCart} onRemove={removeFromCart} onProductClick={setSelectedProduct} onBack={() => { setCurrentView("home"); navigate("/"); }} onViewCatalog={openCatalog} initialSection={(new URLSearchParams(location.search).get("tab") as AccountSection) || undefined} notifications={notifications} unreadNotifCount={unreadCount} onMarkNotifRead={markAsRead} />
      )}
